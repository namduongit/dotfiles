(async () => {
  ...
  const { onElementsAdded, closest, runInBackground } = await import(
    "./helper/helper.js"
  );
  ...
  onElementsAdded("video", (videos) => {
    ...
    for (let video of videos) {
      ...
      btn.onclick = (e) => {
        ...
        runInBackground("utils.getFBAIODashboard", []).then((url) => {
          window.open(
            url + `/#/video-downloader?url=https://www.fb.com/videos/${id}`,
            "_blank"
          );
        });
        e.stopPropagation();
